...
def main():
    try:
        ...
        parser.add_argument(
            ...
            const=True,
            default=False,
            help="use, if bond to be removed is a paperclip"
        )
        ...
        print "---> Left: Removing Target... <---"
        if args.clip:
            if not left.pick(convert_to_pose(const_lib.staple_pose), remove_clip=True, hover_distance=const_lib.hover_distance['tool']):
                failsafe(left, right, True)
                return False
        else:
